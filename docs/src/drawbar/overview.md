...
| **Status bar** | The last thing that happened. Click it to open the log. The size is at the right. |
...
Between them is the size: **Default**, or how many steps from it, from −3 to +4.
Click it to return to Default. **View** has the same three commands, and the
desktop app also takes ⌘+, ⌘− and ⌘0. In a browser those keys zoom the page
instead. The size is kept between sessions.
