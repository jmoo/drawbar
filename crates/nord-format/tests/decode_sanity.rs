...
/// Root of the corpus checkout — same resolution as `tests/corpus`.
...
fn aux_matches_one_of_the_three_documented_shapes() {
    ...
    let mut seen = 0usize;
    for path in every_cbin(&corpus_root()) {
...
fn ne5_backup_dependency_ids() {
    let backup = corpus_root().join("ne5/usb/backup/full_backup");
...
fn ne5l_files() -> Vec<PathBuf> {
    let paths = files_with(&corpus_root().join("ne5"), "ne5l");
...
fn sample_specimens() -> Vec<(PathBuf, String)> {
    let dir = corpus_root().join("ne5/samples");
...
fn nsmp_bad_checksum_is_refused() {
    let mut bytes = read(corpus_root().join("ne5/samples/D1-one-zone.nsmp")).unwrap();
