...
/// The corpus variable set with the feature off means the corpus suites did not run.
#[test]
fn corpus_env_without_the_corpus_feature_is_a_mistake() {
    #[cfg(not(feature = "corpus"))]
    assert!(
        std::env::var_os("NORD_CORPUS_ROOT").is_none(),
        "NORD_CORPUS_ROOT is set but --features corpus is off: the corpus suites did \
