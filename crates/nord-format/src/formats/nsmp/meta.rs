//! The `meta` section: the length of the wide chain ahead of it.
//!
...
//! ```
//!
//! The length counts from the `NSMP` tag to the `meta` header, which is the whole
//! body except `meta` itself. It is the only place a wide file states its own size.
//! ⚠️ A writer that resizes any section must restate it.
//!
...
/// Payload offset of the chain length.
        ...
        let chain_len = payload
            ...
            .ok_or_else(|| {
                ParseError::AssertFail(format!(
                    "meta section is {} bytes, too short for its chain length",
                    payload.len()
                ))
            })?
            .try_into()
...
mod tests {
    ...
    #[test]
    fn a_short_or_unknown_meta_is_refused() {
