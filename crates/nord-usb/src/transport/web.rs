...
//!
//! **Confirmed on hardware** (Chrome on macOS, a real Electro 5, through
//! `crates/drawbar`) for reads *and* writes: open, claim-by-class, both transfer
//! directions across whole transactions (session open, STATUS, INFO, the write
//! sequence, close), the masked endpoint numbering, and release/close handing the
//! interface back to other hosts. Programs and sample instruments have been sent to
//! the instrument this way. **Still not confirmed:** the `select_configuration`
//! branch (it only runs when the OS left the device unconfigured, which macOS does
//! not), multi-chunk bulk reads, and the zero-length-packet edge on `read` — each is
